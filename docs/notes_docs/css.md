...
::: tip
Theme components are subject to the same.
:::
...
`title`, `description` and `base` are copied from respective fields in `.vuepress/config.js`. `pages` contains an array of metadata objects for each page, including its path, page title (explicitly specified in or inferred from the first header on the page), and any YAML front matter data in that file.
...
::: tip
  `lastUpdated` is the UNIX timestamp of this file's last git commit, for more details, refer to.
