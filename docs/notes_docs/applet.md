...
Because VuePress applications are server-rendered in Node.js when generating static builds, any Vue usage must conform to the . In short, make sure to only access Browser / DOM APIs in `beforeMount` or `mounted` hooks.
...
Any `*.vue` files found in `.vuepress/components` are automatically registered as components. For example:
...
For pre-processors that do not have built-in webpack config support, you will need to in addition to installing the necessary dependencies.
:::
...
The compiled content of the current `.md` file being rendered. This will be very useful when you use.
...
- custom-themes.md#content-outlet
